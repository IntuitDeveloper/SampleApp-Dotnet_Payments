...
namespace OAuth2_Dotnet_UsingSDK
{
    public partial class Default : System.Web.UI.Page
    {
        ...
        protected async void ImgPaymentsAPICall_Click(object sender, ImageClickEventArgs e)
        {
            if (dictionary.ContainsKey("realmId"))
            {
                ...
                {
                    //call Payments api
                    await paymentsApiCall(dictionary["accessToken"], dictionary["refreshToken"], dictionary["realmId"]);
                }
            }
            else
            {
        ...
        public async System.Threading.Tasks.Task doOAuth(string callMadeBy)
        {
            ...
            if (callMadeBy == "C2QB") //C2QB scopes
            {
                ...
                scopeVal =  OidcScopes.Payment.GetStringValue();
            }
        ...
        public async System.Threading.Tasks.Task paymentsApiCall(string access_token, string refresh_token, string realmId)
        {
            try
            {
                ...
                if (realmId != "")
                {
                    ...
                    JObject cardChargeResponse = executePaymentsCharge(cardToken, realmId, access_token, refresh_token);
